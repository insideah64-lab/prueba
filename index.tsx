...
async function interpretAction(action: string, state: GameState): Promise<any> {
  if (!Bun.env.GROQ_API_KEY) return { narrative: "IA offline", hpChange: 0, xpChange: 0 };
  try {
    const prompt = `Eres árbitro de Aethelraed MUD. Jugador: ${state.playerName} (${state.playerClass} Lv${state.classLvl}). Acción: "${action}". Responde JSON: {"narrative":"...","hpChange":0,"xpChange":25}`;
    ...
    if (jsonMatch) {
      ...
      return { narrative: parsed.narrative || "Algo sucede", hpChange: parsed.hpChange || 0, xpChange: parsed.xpChange || 0 };
    }
  } catch (e) {
    console.error("Groq:", e);
  }
  return { narrative: "El silencio envuelve tu acción", hpChange: 0, xpChange: 10 };
}
...
<style>
.mud-log{font-family:'Courier New',monospace;background:#0a0e27;color:#00d084;border:1px solid #1a2332;overflow-y:auto}
...
<h1 class="text-3xl font-bold neon-text mb-1">Aethelraed: Infinity Engine</h1>
...
<div class="mud-log rounded p-3 flex-1"><div id="gameLog"><p class="text-yellow-400">[Conectando...]</p></div></div>
<div class="flex gap-2">
<textarea id="cmd" placeholder="Tu accion..." class="flex-1 mud-input px-3 py-2 rounded text-sm" rows="2"></textarea>
...
<div class="flex justify-between"><span class="text-gray-500">Clase:</span><span id="class">---</span></div>
<div class="flex justify-between"><span class="text-gray-500">Oficio:</span><span id="job">---</span></div>
<div class="flex justify-between"><span class="text-gray-500">Ubicacion:</span><span id="loc">---</span></div>
</div>
</div>
<div class="bg-gray-900 border border-gray-700 rounded p-3">
...
app.post("/api/init", async (c) => {
  ...
  gameState.log.push("Despiertas en la Taberna del Oxido...");
  saveGame(gameState);
...
app.post("/api/action", async (c) => {
  ...
  if (!gameState.isAlive) {
    gameState.log.push("Estas muerto.");
    return c.json(gameState);
  }
  ...
  if (gameState.classXp >= gameState.classLvl * 100) {
    ...
    gameState.log.push("Ascension de clase! Nivel " + gameState.classLvl);
  }

  if (gameState.jobXp >= gameState.jobLvl * 100) {
    gameState.jobLvl++;
    gameState.jobXp = 0;
    gameState.log.push("Maestria! Nivel " + gameState.jobLvl);
  }

  if (gameState.hp <= 0) {
    gameState.isAlive = false;
    gameState.log.push("Has caido.");
  }

  if (Math.random() < 0.1) {
    const events = ["Niebla...", "Grito lejano...", "Tiembla el suelo..."];
    gameState.log.push(events[Math.floor(Math.random() * events.length)]);
  }
