...
INSTRUCCIONES:
1. Interpreta la acción del jugador de forma narrativa
2. Decide el impacto: daño (-5 a -20), XP (10-50), ítems (0-2)
3. Mantén un tono gótico y épico
4. Responde en ESPAÑOL
5. Sé breve (máximo 2 líneas)

RESPONDE EXACTO en JSON:
{"narrative":"Descripción","hpChange":-10,"xpChange":25,"itemsGained":[]}`;
    ...
    if (content.type === "text") {
      ...
      if (jsonMatch) {
        ...
        return {
          narrative: result.narrative || "Algo sucede...",
        ...
        <div id="app" class="min-h-screen flex flex-col">
          <div class="mud-header p-4 shadow-lg">
            ...
            <p class="text-xs text-gray-400">
              Escribe tus acciones. La IA las interpreta.
            </p>
          </div>

          <div class="flex-1 flex gap-3 p-3 max-w-full mx-auto w-full overflow-hidden">
            {/* Main */}
            <div class="flex-1 flex flex-col gap-3 min-w-0">
              {/* Log */}
              <div class="mud-log rounded p-3 flex-1 overflow-y-auto">
                <div id="gameLog">
                  <p class="text-yellow-400">[Conectando...]</p>
                </div>
              </div>
              ...
              <div class="flex gap-2 flex-shrink-0">
                <textarea
                  id="commandInput"
                  placeholder="Tu acción..."
                  class="flex-1 mud-input px-3 py-2 rounded text-sm"
                  rows="2"
                ></textarea>
                <button
                  id="sendBtn"
                  class="px-4 py-2 bg-green-700 hover:bg-green-600 rounded font-bold text-sm"
                >
                  Ejecutar
                </button>
              </div>
              ...
              <div class="bg-gray-900 border border-gray-700 rounded p-3">
                ...
                <div class="space-y-1 text-xs">
                  <div class="flex justify-between">
                    ...
                    <span id="playerClass" class="text-blue-400">
                      {gameState?.playerClass || "---"}
                    </span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-500">Oficio:</span>
                    <span id="playerJob" class="text-purple-400">
                      {gameState?.playerJob || "---"}
                    </span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-500">Ubicación:</span>
                    <span id="location" class="text-cyan-400">
                      {gameState?.location || "---"}
                    </span>
                  </div>
                </div>
              ...
              <div class="bg-gray-900 border border-gray-700 rounded p-3">
                <div class="space-y-2 text-xs">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span id="gold" class="text-yellow-400">
                      {gameState?.gold || 0}
                    </span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-500">XP:</span>
                    <span id="exp" class="text-green-400">
...
app.post("/api/action", async (c) => {
  ...
  if (Math.random() < 0.1) {
    const events = [
      "Una niebla errante desciende...",
      "El suelo tiembla...",
      "Una sombra misteriosa aparece...",
    ];
