...
async function interpretAction(action: string, state: GameState): Promise<{ narrative: string; hpChange: number; xpChange: number; itemsGained: Item[] }> {
  if (!Bun.env.GROQ_API_KEY) {
    return { narrative: "La IA está offline.", hpChange: 0, xpChange: 0, itemsGained: [] };
  }

  try {
    const prompt = `Eres árbitro de un MUD. El jugador ${state.playerName} (${state.playerClass} Lv${state.classLvl}) hace: "${action}"
...
{"narrative":"Breve narrativa épica","hpChange":-10,"xpChange":25,"itemsGained":[]}`;
    ...
    if (jsonMatch) {
      ...
      return { narrative: result.narrative || "Algo sucede...", hpChange: result.hpChange || 0, xpChange: result.xpChange || 0, itemsGained: result.itemsGained || [] };
    }
  } catch (error) {
    console.error(error);
  }
          ...
          <div class="flex-1 flex gap-3 p-3 max-w-full overflow-hidden">
            <div class="flex-1 flex flex-col gap-3 min-w-0">
              <div class="mud-log rounded p-3 flex-1 overflow-y-auto">
                <div id="gameLog"><p class="text-yellow-400">[Conectando...]</p></div>
              </div>

              <div class="flex gap-2 flex-shrink-0">
                <textarea id="commandInput" placeholder="Tu acción..." class="flex-1 mud-input px-3 py-2 rounded text-sm" rows="2"></textarea>
                <button id="sendBtn" class="px-4 py-2 bg-green-700 hover:bg-green-600 rounded font-bold text-sm">Ejecutar</button>
              </div>
            </div>

            <div class="w-72 flex flex-col gap-2 overflow-y-auto flex-shrink-0">
              <div class="bg-gray-900 border border-gray-700 rounded p-3">
                <h2 class="text-lg font-bold neon-text mb-2">{gameState?.playerName || "---"}</h2>
                <div class="space-y-1 text-xs">
                  <div class="flex justify-between"><span class="text-gray-500">Clase:</span><span id="playerClass" class="text-blue-400">{gameState?.playerClass || "---"}</span></div>
                  <div class="flex justify-between"><span class="text-gray-500">Oficio:</span><span id="playerJob" class="text-purple-400">{gameState?.playerJob || "---"}</span></div>
                  <div class="flex justify-between"><span class="text-gray-500">Ubicación:</span><span id="location" class="text-cyan-400">{gameState?.location || "---"}</span></div>
                </div>
              </div>
              ...
              <div class="bg-gray-900 border border-gray-700 rounded p-3">
                ...
                <p id="hpText" class="text-xs">{gameState?.hp || 0}/100</p>
              </div>

              <div class="bg-gray-900 border border-gray-700 rounded p-3">
                <div class="space-y-2 text-xs">
                  <div class="flex justify-between"><span class="text-gray-500">Clase Lv:</span><span id="classLvl">{gameState?.classLvl || 1}</span></div>
                  <div class="flex justify-between"><span class="text-gray-500">Oficio Lv:</span><span id="jobLvl">{gameState?.jobLvl || 1}</span></div>
                  <div class="flex justify-between"><span class="text-gray-500">Oro:</span><span id="gold" class="text-yellow-400">{gameState?.gold || 0}</span></div>
                  <div class="flex justify-between"><span class="text-gray-500">XP:</span><span id="exp" class="text-green-400">{gameState?.experience || 0}</span></div>
                </div>
              </div>
              ...
              <div class="bg-gray-900 border border-gray-700 rounded p-3">
                ...
                <div class="space-y-1 text-xs">
                  <div class="flex justify-between"><span>Barones:</span><span id="baron" class="text-gray-400">0</span></div>
                  <div class="flex justify-between"><span>Culto:</span><span id="cult" class="text-gray-400">0</span></div>
                </div>
              </div>
              ...
              <div class="bg-gray-900 border border-gray-700 rounded p-3">
                ...
                <div id="inventoryList" class="space-y-1 text-xs max-h-40 overflow-y-auto">
                  {gameState?.inventory.map((item, idx) => (
                    <div key={idx} class="flex justify-between"><span>{item.name}</span><span class="text-cyan-400">{item.rarity}</span></div>
                  ))}
                </div>
              </div>
            </div>
        ...
        <script>
          {`
            async function initGame() {
              const name = prompt("Tu nombre:", "Darian");
              if (!name) return;
              const res = await fetch("/api/init", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) });
...
app.post("/api/init", async (c) => {
  ...
  gameState.log.push("Escribe tus acciones libremente. La IA las interpretará.");
  gameState.log.push("═══════════════════════════════════════════════════════════");
...
app.post("/api/action", async (c) => {
  ...
  if (!gameState.isAlive) {
    gameState.log.push("💀 Estás muerto. Tu aventura terminó.");
    return c.json(gameState);
  }
  ...
  if (gameState.jobXp >= gameState.jobLvl * 100) {
    gameState.jobLvl++;
    gameState.jobXp = 0;
    gameState.log.push(`🎯 ¡Maestría profesional! Nivel ${gameState.jobLvl}`);
  }
  ...
  if (Math.random() < 0.1) {
    const events = ["Una niebla errante desciende...", "El suelo tiembla...", "Una sombra misteriosa aparece..."];
