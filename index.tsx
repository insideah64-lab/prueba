      ...
      <body class="bg-gray-950 text-gray-100">
        ...
        <script dangerouslySetInnerHTML={{__html: `
          ...
          function updateUI(state) {
            document.getElementById("name").textContent = state.playerName;
            document.getElementById("class").textContent = state.playerClass;
            document.getElementById("job").textContent = state.playerJob;
            document.getElementById("loc").textContent = state.location;
            document.getElementById("clvl").textContent = state.classLvl;
            document.getElementById("jlvl").textContent = state.jobLvl;
            document.getElementById("gold").textContent = state.gold;
            document.getElementById("xp").textContent = state.experience;
            ...
            log.innerHTML = state.log.map(function(m) { return "<p>" + m + "</p>"; }).join("");
            log.scrollTop = log.scrollHeight;

            const inv = document.getElementById("inv");
            inv.innerHTML = state.inventory.map(function(item) {
              return "<div class=\\"flex justify-between\\"><span>" + item.name + "</span><span class=\\"text-cyan-400\\">" + item.rarity + "</span></div>";
            }).join("");
          }

          document.getElementById("btn").addEventListener("click", sendCmd);
